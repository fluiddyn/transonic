...
def main():
    """Minimal layer above the Cython commandline"""

    assert sys.argv[0].endswith(
        os.path.sep.join(("transonic_cl", "run_cython.py"))
    )
    ...
    print(f"Cythonizing {path}", flush=True)
    ...
    if path_out.exists():
        print(f"File {path_out.absolute()} created by Cython")
    else:
        logger.error(
            f"Error! File {path_out.absolute()} has not been created by Cython"
        )
        try:
            completed_process
        except NameError:
            ...
            if completed_process.stdout:
                print(f"Cython stdout:\n{completed_process.stdout}")
            if completed_process.stderr:
                logger.error(f"Cython stderr:\n{completed_process.stderr}")
